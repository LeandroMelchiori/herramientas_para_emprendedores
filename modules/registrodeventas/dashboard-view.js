...
function populateMonthlySelector(sales) {
  const select = document.getElementById('select-mes-panel');
  if (!select) return '';
  const previous = select.value;
  const months = availableMonths(sales);
...
function renderizarPanelMensual() {
  ...
  const comparison = AppSalesDashboard.compareMonths(sales, month);
  const data = comparison.current;
